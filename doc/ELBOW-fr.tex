...
unique livré (\code{robust\_knee}, \code{robust\_elbow}), la recherche
multi-coudes qui alimente \code{robust\_knees} et le socle commun de
vraisemblance gaussienne sur lequel repose chaque critère de sélection de
modèle du paquet. Les références se trouvent dans \code{references.bib}.
Pour chaque affirmation, on précise s'il s'agit d'une citation vérifiée,
d'une citation rapportée par une source secondaire, ou d'une construction
propre à ce projet, selon la convention de sourçage adoptée dans
l'ensemble du dépôt.
...
Une introduction dérive, une seule fois, la machinerie qui va de la
vraisemblance brute à un score borné et que chaque critère ultérieur
réutilise~: pourquoi $\mathrm{BIC}$, $\mathrm{mBIC}$ et $\mathrm{ICL}$
reposent tous sur le logarithme naturel, et comment un pire cas
délibérément pessimiste, plutôt qu'un nombre brut non borné, transforme
n'importe lequel d'entre eux en un score $[0,1]$ justifié et
interprétable. À partir de là, toute la note tourne autour d'une seule
question posée trois fois, avec un réalisme croissant~: que signifie
géométriquement \emph{un} coude (partie~\ref{part:un}), que signifie
géométriquement \emph{plusieurs} coudes (partie~\ref{part:plusieurs}) et
comment faire confiance à l'une ou l'autre réponse une fois les données
bruitées (partie~\ref{part:bruit})~?
\end{abstract}
...
Sur une courbe à rendements décroissants, un coude marque l'endroit où
chaque unité supplémentaire investie rapporte de moins en moins. Les
détecteurs existants savent bien proposer un point~; ce qui leur manque
est une notion de confiance, si bien qu'une estimation ponctuelle isolée
sur une courbe bruitée est facile à surinterpréter. \code{elbow-helper}
enveloppe un repéreur réécrit de zéro dans une procédure de décision
prudente et refuse de trancher franchement quand les preuves sont trop
minces.

Cette note dérive, à partir des principes de base, chaque brique
mathématique que le paquet met en œuvre. Plusieurs de ses critères de
sélection de modèle, des scores qui récompensent un bon ajustement tout en
pénalisant la complexité superflue, remontent à la même log-vraisemblance
gaussienne~: le $\mathrm{BIC}$ à coude unique, le critère d'information
bayésien (section~\ref{sec:bic-vc})~; le $\mathrm{BIC}(k)$ à plusieurs
coudes de la section~\ref{sec:several-bic}~; son cousin plus strict, le
BIC modifié, $\mathrm{mBIC}$ (section~\ref{sec:mbic})~; et une version qui
facture aussi l'ambiguïté des frontières de segments, la vraisemblance
complétée intégrée, $\mathrm{ICL}$ (section~\ref{sec:icl}). Cette
introduction dérive ce socle commun une seule fois, en logarithme naturel
de bout en bout, avant qu'aucune de
ces sections n'y ajoute son propre terme de pénalité.

\section{De la vraisemblance brute à un score borné}
\label{sec:likelihood-intro}

\textbf{La notion, en bref.} La \emph{vraisemblance} désigne
habituellement la densité jointe des données observées, lue comme une
fonction du paramètre plutôt que des données, $L_{\text{brute}}(\theta)
:= \prod_{i=1}^n p(y_i \mid x_i, \theta)$. Cette note prend au contraire
la vraisemblance \emph{par observation} dès le départ, une moyenne
\emph{exponentiée} de log-densités plutôt qu'un produit brut,
...
racine $n$-ième de $L_{\text{brute}}$. Cet écart par rapport à la
définition classique, et pourquoi il est principiel plutôt qu'une
simple remise à l'échelle de confort, fait l'objet d'une note
compagne, \code{LIKELIHOOD-fr.tex}, séparée parce que ce socle est
général et ne dépend en rien de l'ajustement de courbes~: l'inégalité
de Jensen et un raffinement indexé par une température emprunté à la
mécanique statistique à l'équilibre, les grandes déviations de
Cram\'er et la martingale de Wald, un compte rendu précis de $L$
comme estimateur statistique avec son biais et sa variance dérivés et
vérifiés par simulation, et comment cette même construction se lit
sur un modèle de classification catégorielle (avec sa propre
simulation travaillée) aussi bien que sur le modèle de régression
gaussienne dérivé plus bas. Cette note construit directement
dessus~: chaque critère de la famille $\mathrm{BIC}$ ci-dessous hérite
de l'instanciation gaussienne de $L$. Le test séquentiel fondé sur la
permutation de la section~\ref{sec:fwer} est exactement le cas que
\code{LIKELIHOOD-fr.tex} identifie où la borne martingale classique de
Wald ne s'applique pas directement, une alternative composite plutôt
qu'entièrement spécifiée à chaque étape.

Sous un modèle génératif $y_i = f(x_i;\theta) + \varepsilon_i$ avec $\varepsilon_i$
i.i.d.\ $\mathcal N(0,\sigma^2)$ ($f$ n'importe lequel des modèles de
courbe que cette note ajuste, une droite simple, une ligne brisée, ou une
ligne brisée à $k$ segments~; $\theta$ ses paramètres de forme)~:
...
Le produit de $n$ préfacteurs identiques et le produit d'exponentielles
s'effondrent proprement sous la racine $1/n$~: un produit d'exponentielles
est une exponentielle d'une somme, et cette somme divisée par $n$ est une
moyenne.
...
l'erreur quadratique moyenne par point, $\mathrm{MSE}(\theta) :=
\frac{1}{n}\sum_i (y_i - f(x_i;\theta))^2$. $L$
est déjà une expression en $\exp$, en base $e$, directement issue de la
définition même de la densité gaussienne, avant qu'un analyste choisisse
de prendre un logarithme par commodité. Prendre le $\ln$ de $L$
n'introduit donc pas de base~: il défait celle qui est déjà là~:
...
Minimiser $\mathrm{MSE}$ sur $\theta$ à $\sigma^2$ fixé revient à
maximiser $\ell$ sur $\theta$~: c'est pourquoi les moindres carrés
ordinaires et le maximum de vraisemblance gaussien forment une seule et
même procédure. En profilant $\sigma^2$ à sa valeur la mieux soutenue par
les données, l'estimateur du maximum de vraisemblance (EMV) $\hat\sigma^2
= \mathrm{MSE}$, puis en exponentiant $-2\ell$, l'échelle conventionnelle
d'un critère d'information, on défait le $\ln$ et on obtient une seule
quantité limpide par observation, la mesure que la théorie de
l'information appelle perplexité~:

\begin{equation}
\mathrm{PPL}(\theta) := \exp\!\big(-2\,\ell(\theta,\hat\sigma^2)\big) = 2\pi e \cdot \mathrm{MSE}(\theta)
\end{equation}

En remontant cette perplexité par observation à l'échelle de
l'échantillon complet à $n$ points, on retrouve la log-vraisemblance
négative, $\mathrm{NLL}$, dont part toute formule de la famille
$\mathrm{BIC}$ dans cette note, écrite ici via la somme des carrés des
résidus, $RSS(\theta) := \sum_i (y_i - f(x_i;\theta))^2$, le numérateur
non moyenné que $\mathrm{MSE}$ divise par $n$~:

\begin{equation}
\mathrm{NLL} := n \ln \mathrm{PPL}(\theta) = n \ln\!\left(\frac{RSS}{n}\right) + n(1 + \ln 2\pi)
\end{equation}

le même terme $n\ln(RSS/n)$ par lequel s'ouvre toute formule de la
famille $\mathrm{BIC}$ dans cette note, mesuré en \emph{nats}~: l'analogue
en logarithme naturel du bit, un nat est l'information gagnée par un
événement $1/e$ fois moins probable que la certitude, l'unité dans
laquelle toute quantité construite avec $\ln$ plutôt que $\log_2$
s'exprime implicitement. Chaque log-vraisemblance, chaque
$\mathrm{NLL}$, $\mathrm{BIC}$, $\mathrm{mBIC}$ et $\mathrm{ICL}$ dans
cette note est un compte de nats, pas un nombre sans unité, ce qui
explique en partie pourquoi une valeur brute comme «~$282,4$ nats~» ne se
lit pas comme un score borné~: les nats s'accumulent avec $n$ et avec
l'échelle propre de la log-vraisemblance, exactement la motivation
derrière les scores bornés dérivés plus bas.

\textbf{Cas extrêmes.} Le $\mathrm{PPL}$ est une
amélioration véritable par rapport au $\mathrm{NLL}$ brut dont il est
issu, sur exactement un côté~: le $\mathrm{NLL}$ parcourt $(-\infty,
+\infty)$, mais exponentier corrige automatiquement la queue inférieure,
$\mathrm{PPL} = 2\pi e \cdot \mathrm{MSE}(\theta) \ge 0$ toujours, avec
$\mathrm{PPL} = 0$ à l'ajustement parfait ($\mathrm{MSE} \to 0$)~: la
dégénérescence gaussienne qui justifie l'existence même du BIC, un modèle
avec autant de paramètres que de points pousse le $\mathrm{MSE}$ vers zéro
et sa vraisemblance brute vers l'infini. Le côté supérieur reste non
résolu~: $\mathrm{MSE} \to \infty$ n'est pas un pire cas significatif~; il
est simplement non borné, pas un vrai point de référence près duquel un
ajustement se situerait. La moyenne
d'échantillon $\bar y$ n'est pas non plus une bonne référence de pire
cas~: c'est l'\emph{unique} constante qui \emph{minimise} le
$\mathrm{MSE}$ parmi toutes les constantes, le meilleur prédicteur
trivial qui soit, pas un mauvais. C'est exactement pourquoi normaliser
contre elle (le $R^2$ classique) peut encore donner un résultat négatif~:
un ajustement réel est facile à rendre pire que la seule constante que
rien d'autre ne peut battre. Un pire cas vraiment pessimiste et fini
choisit plutôt le point \emph{observé} le plus difficile à partir duquel
prédire tous les autres~:

\begin{equation}
y_{\text{mauvais}} := \operatorname*{arg\,max}_{y_j \in \{y_1,\dots,y_n\}}
\frac{1}{n}\sum_i (y_i - y_j)^2, \qquad
\mathrm{MSE}_{\text{pire}} := \frac{1}{n}\sum_i (y_i - y_{\text{mauvais}})^2
\end{equation}

Comme $\bar y$ minimise le $\mathrm{MSE}$ de façon unique parmi les
constantes, toute autre constante, une valeur effectivement observée en
premier lieu, obtient un score strictement pire~: $\mathrm{MSE}_{\text{pire}}
> \mathrm{Var}(y)$ toujours. Normaliser contre ce plafond plus difficile à
battre donne à la place

\begin{equation}
1 - \frac{\mathrm{MSE}(\theta)}{\mathrm{MSE}_{\text{pire}}} = 1 - \frac{\mathrm{PPL}(\theta)}{\mathrm{PPL}_{\text{pire}}}
\end{equation}

un score ancré à $1$ pour un ajustement parfait et à $0$ pour un
ajustement aussi mauvais que $y_{\text{mauvais}}$ lui-même, avec une
barre bien plus haute à franchir pour tomber en territoire négatif que ce
que le $R^2$ fondé sur la moyenne offre jamais, puisque
$\mathrm{MSE}_{\text{pire}} > \mathrm{Var}(y)$ par construction, construit
entièrement à partir du $\mathrm{MSE}$ contre un point de référence réel
et délibérément pessimiste, sans facteur de Bayes ni probabilité a
posteriori nulle part dans la dérivation. C'est un cousin plus strict du
$R^2$, pas le $R^2$ lui-même~: le $R^2$ garde $\bar y$ comme référence et
accepte en échange un score parfois négatif. La légende de diagnostic
d'\code{elbow\_helper.plotting} prend une autre voie~: elle convertit le
$\Delta\mathrm{BIC}$ directement en probabilité a posteriori via un
facteur de Bayes \citep{kassraftery1995}, une normalisation alternative
tout aussi valable, construite sur le même $\mathrm{NLL}$ mais différente
de celle dérivée ici.

Toute quantité de la famille $\mathrm{BIC}$ dans cette note hérite de
cette même base naturelle par construction~: chacune est le
$\mathrm{NLL}$ plus une pénalité de complexité, jamais une vraisemblance
redérivée depuis zéro. Changer la base du logarithme n'importe où en aval
reviendrait à redériver la densité gaussienne elle-même dans cette base,
pas à redimensionner un nombre déjà obtenu.

\section{Travaux connexes}
\label{sec:related-work}

\textbf{Heuristiques de coude et d'elbow.} La littérature sur la
détection de coude a un point de référence dominant~: le Kneedle de
Satopaa, Albrecht, Irwin et Raghavan \citep{satopaa2011}, construit
exactement pour le cadre par lequel cette note s'ouvre, un opérateur
système face à une courbe qui a besoin d'une réponse automatique à
« où cela plie-t-il ». Le mécanisme de Kneedle est la même géométrie de
distance à la corde que la section~\ref{sec:diff-curve} construit~:
normaliser la courbe au carré unité, soustraire la droite reliant ses
deux extrémités et appeler coude le point de distance verticale maximale
à cette corde. C'est rapide, peu paramétré, et, sur une courbe propre,
généralement juste. Ce que cela ne fait pas, c'est dire à quel point
c'est juste~: Kneedle renvoie un seul indice, que la courbe soit un coude
manuel ou trois points de bruit pur qui se trouvent osciller dans le bon
sens, sans aucune notion de la façon dont cette réponse changerait sous
un rééchantillonnage, sous une autre échelle de lissage ou face à un
modèle nul sans coude du tout. Cet écart, une estimation ponctuelle sans
confiance attachée, est précisément ce que cette note entière existe à
combler. Le repéreur d'\code{elbow-helper} garde l'intuition géométrique
de Kneedle en son cœur, la courbe de différence normalisée reste
l'origine de chaque coude candidat, mais traite ce candidat comme une
hypothèse à mettre à l'épreuve
(sections~\ref{sec:multiechelle}--\ref{sec:null-test}) plutôt que comme une
réponse à renvoyer telle quelle.

\textbf{Détection de points de rupture et segmentation.} Une fois la
question passée d'un coude à plusieurs, la littérature pertinente n'est
plus la détection de coude mais la détection de points de rupture. Elle
se scinde selon une ligne coût contre flexibilité que cette note doit
trancher~: une recherche exacte qui ne vaut son coût que jusqu'à un
$k_{\max}$ borné ou une approximation gloutonne qui passe mieux à
l'échelle mais peut s'enfermer dans une erreur précoce. L'algorithme de
programmation dynamique élaguée de Killick, Fearnhead et Eckley, Pruned
Exact Linear Time (PELT) \citep{killick2012}, se situe du côté exact~:
un programme dynamique pénalisé, élagué de sorte que les frontières
candidates provablement sous-optimales soient écartées avant même d'être
évaluées, retrouvant la même segmentation optimale qu'une recherche non
élaguée, à coût linéaire plutôt que quadratique, avec $k$ lui-même
déterminé par la pénalité plutôt que fixé à l'avance. La
section~\ref{sec:dp} repose sur le même argument de sous-structure
optimale, restreint ici à un petit $k_{\max}$ explicitement borné (peu de
courbes réelles ont plausiblement plus d'une poignée de vraies pliures)
plutôt qu'au $k$ non borné et piloté par pénalité de PELT, ce qui est
justement ce qui garde la recherche assez petite pour faire tourner
par-dessus les couches de bootstrap et de permutation dont PELT lui-même
n'a pas d'équivalent. La segmentation binaire sauvage de Fryzlewicz
\citep{fryzlewicz2014} se situe de l'autre côté de cette ligne~: une
procédure de division récursive et gloutonne sur des sous-intervalles
tirés au hasard, plus rapide que la recherche exacte mais susceptible de
figer tôt une mauvaise coupure qu'elle ne pourra plus jamais reconsidérer,
exactement le mode d'échec que la section~\ref{sec:greedy} démontre sur
un exemple construit et la raison pour laquelle ce projet évalue la
segmentation gloutonne comme l'alternative plus rapide mais moins bonne,
plutôt que comme le choix par défaut. Yao \citep{yao1988} est le premier
à relier directement le comptage de points de rupture au critère de
Schwarz, l'article dont le $\mathrm{BIC}(k)$ à plusieurs coudes de la
section~\ref{sec:several-bic} retrace l'ascendance, même si, comme cette
section le montre, les hypothèses du BIC nu ne survivent pas intactes au
contact d'un emplacement de rupture inconnu.

\textbf{Critères de sélection de modèle.} Chaque critère que cette note
construit répond à la même question, la complexité supplémentaire
en vaut-elle la peine, avec des notions de moins en moins naïves de ce
que « complexité » coûte. Schwarz \citep{schwarz1978} fournit le point de
départ~: une approximation de Laplace à la vraisemblance marginale d'un
modèle, qui facture un $\frac{1}{2}\ln n$ fixe par paramètre libre,
correct pour les familles paramétriques régulières mais qui suppose
silencieusement absente exactement l'irrégularité qu'introduit
l'emplacement d'un point de rupture, une hypothèse dont la
section~\ref{sec:several-bic} montre l'échec concret, pas seulement en
principe. Zhang et Siegmund \citep{zhangsiegmund2007} diagnostiquent
cette même irrégularité et la corrigent par une pénalité sensible à la
longueur des segments, le $\mathrm{mBIC}$ de la section~\ref{sec:mbic},
même si leur propre dérivation laisse une véritable ambiguïté de signe
que cette note résout empiriquement plutôt que par hypothèse (encore la
section~\ref{sec:mbic}). Biernacki, Celeux et Govaert
\citep{biernacki2000} abordent la complexité sous un angle entièrement
différent, développé à l'origine pour choisir le nombre de composantes
d'un modèle de mélange, pas du tout un compte de points de rupture,
facturant non seulement les paramètres mais aussi l'ambiguïté de
l'affectation qui en résulte~: deux amas qui se chevauchent et sont
difficiles à séparer coûtent plus cher que deux amas propres, même à
ajustement égal. Rigaill, Lebarbier et Robin \citep{rigaill2012}
fournissent la machinerie qui rend praticable le transfert de cette idée
aux points de rupture, une distribution a posteriori exacte par passage
avant-arrière sur tout l'espace des segmentations discrètes,
structurellement la même récurrence que l'algorithme avant-arrière d'un
modèle de Markov caché, simplement sur des segmentations plutôt que sur
des états cachés, accélérée plus tard par Cleynen, Luong, Rigaill et Nuel
\citep{cleynen2013}. Le récit que fait la section~\ref{sec:icl} de cette
adaptation inclut un \emph{bug} que les tests de ce projet ont attrapé
directement~: une première version qui laissait le terme de vraisemblance
intégrée dominer la correction d'entropie et surestimer sur du bruit pur,
un mode d'échec auquel la littérature originale sur les modèles de
mélange n'a pas à faire face de la même façon. Kass et Raftery
\citep{kassraftery1995} se situent légèrement à l'écart de toute cette
famille~: non pas une pénalité de complexité, mais une lecture
décisionnelle d'une différence de log-vraisemblance comme un facteur de
Bayes approché, la normalisation alternative fondée sur une sigmoïde que
la section~\ref{sec:likelihood-intro} dérive puis met de côté au profit
du score ancré sur le pire cas utilisé dans le reste de cette note.

\textbf{Tests séquentiels et multi-échelles.} Une troisième famille de
méthodes répond directement à la question « ce point de rupture est-il
réel », comme un test d'hypothèse plutôt que comme un score pénalisé par
la complexité. Davies \citep{davies1977} identifie la difficulté centrale
que tout test de ce genre doit résoudre~: l'emplacement du point de
rupture est un paramètre de nuisance qui n'existe que sous l'hypothèse
alternative, si bien que l'asymptotique usuelle du rapport de
vraisemblance ne s'applique pas, exactement le problème de pivot que le
test séquentiel de la section~\ref{sec:fwer} contourne en utilisant un
nul de permutation plutôt qu'asymptotique. Le Simultaneous MUlti-scale
Change-point Estimator de Frick, Munk et Sieling (SMUCE)
\citep{frick2014} résout un problème apparenté mais plus ambitieux, une
seule affirmation de confiance qui tient simultanément à chaque échelle
de lissage à la fois, sensiblement plus de machinerie que ce dont la
section~\ref{sec:fwer} a besoin pour un test séquentiel, un point de
rupture à la fois, dont la validité se vérifie directement par simulation
plutôt que par la théorie asymptotique. Les deux ont été considérés puis
écartés au profit de la construction fondée sur la permutation
effectivement livrée, un compromis délibéré~: moins de généralité
théorique contre une garantie que ce projet a pu vérifier
empiriquement plutôt que simplement citer.

\textbf{Statistique robuste et classique.} Le pipeline à coude unique
s'appuie sur des outils plus anciens et plus étroits partout où une
méthode classique, robuste et bien comprise existe déjà, plutôt que d'en
inventer une sur mesure. La corrélation de rang de Spearman
\citep{spearman1904} filtre la présence d'une tendance monotone avant
toute recherche de coude, choisie plutôt qu'une corrélation de Pearson
précisément parce qu'elle demande seulement si $y$ évolue de façon
cohérente avec $x$, pas si ce mouvement est linéaire, si bien qu'un
véritable coude courbe ne peut pas la tromper en lui faisant conclure à
l'absence de tendance. Theil \citep{theil1950} et Sen \citep{sen1968},
développés à vingt ans d'écart et combinés depuis en un seul estimateur,
fournissent l'estimation de pente derrière le contraste de Theil-Sen~: la
médiane de toutes les pentes par paire, une seule valeur aberrante
extrême parmi de nombreux points ordinaires ne peut pas déplacer une
médiane comme elle déplacerait un ajustement par moindres carrés
ordinaires. Le bootstrap d'Efron \citep{efron1979} est le principe de
rééchantillonnage derrière le test de stabilité de la
section~\ref{sec:bic-vc}, choisi plutôt qu'un intervalle de confiance
asymptotique en forme close précisément parce que le coude candidat de ce
pipeline a déjà survécu à plusieurs étapes de sélection non linéaires et
non lisses au moment où une affirmation de confiance est nécessaire,
exactement le cadre où la normalité asymptotique est la plus difficile à
justifier et le rééchantillonnage le plus facile.

\textbf{Méthodologie.} L\'opez de Prado \citep{lopezdeprado2018financial}
défend longuement, à partir de séries temporelles financières plutôt que
d'ajustement de courbes, des blocs de validation croisée contigus plutôt
que mélangés dès lors que des observations voisines sont corrélées,
puisqu'une répartition mélangée laisse fuir dans l'estimation hors
échantillon d'un point l'information de ses propres voisins. Le même
argument se transpose directement à l'axe des $x$ d'une courbe~: des
points voisins sur une courbe lisse quelconque se ressemblent pour la
même raison structurelle que des jours voisins d'une série de prix, ce
pourquoi la validation croisée par blocs d'\code{elbow-helper}
(section~\ref{sec:bic-vc}) met de côté des blocs contigus de $x$ plutôt
que des points tirés au hasard.

\textbf{Pour aller plus loin.} Hastie, Tibshirani et Friedman
\citep{hastie2009esl}, Bishop \citep{bishop2006prml}, Bouveyron, Celeux,
Murphy et Raftery \citep{bouveyron2021mbc}, Cormen, Leiserson, Rivest et
Stein \citep{cormen2022algorithms} et Dasgupta, Papadimitriou et
Vazirani \citep{dasgupta2006algorithms} sont cités tout au long de cette
note comme traitements pédagogiques accessibles, non comme l'origine
d'une affirmation particulière ci-dessus~; quelques-uns, tirés de la
\href{https://deraison.ai/ai-books}{liste de livres} personnellement
tenue par l'auteur, ne sont indiqués que là où directement pertinents.
...
\section{La bosse de la courbe de différence~: ce que « coude » veut dire, précisément}
...
d'une droite, ce qui correspond à l'idée intuitive de « coude ». Cette image
se transforme en algorithme \citep{satopaa2011}~: soustraire la diagonale,
repérer le sommet de la bosse, l'appeler le coude. \code{locator.py}, dans
\code{elbow-helper}, en est un portage NumPy réécrit de zéro~; voir la
section Remerciements du \code{README.md} du projet pour l'origine de cette
implémentation.
...
convexe décroissant emblématique. Il vaut la peine de le voir tourner de
...
\textbf{Lire la légende de preuves, avant sa propre dérivation.} La
figure ci-dessous est la première de cette note avec la légende de
diagnostic complète qu'\code{elbow\_helper.plotting} dessine à côté de
chaque coude confirmé, cinq nombres que les parties~\ref{part:plusieurs}
et~\ref{part:bruit} dérivent proprement plus loin~; en langage clair par
avance~: la \emph{probabilité de détection} (section~\ref{sec:bootstrap})
est la fréquence à laquelle le même coude réapparaît sous une reprise par
bootstrap des résidus~: proche de $100\%$ signifie que le coude n'est pas
un artefact de ce seul tirage de bruit. Le \emph{p (nul)} (valeur $p$ du
test nul, section~\ref{sec:null-test}) est le degré de surprise qu'aurait
le coude observé si la vraie courbe était une droite~: petit signifie
qu'une explication par une simple droite est peu plausible. Le
\emph{contraste de pente} (section~\ref{sec:theilsen}) est un score
$[0,1]$ pour la netteté du changement de pente au coude~: proche de
$100\%$ signifie que les deux côtés pointent dans des directions
essentiellement opposées ou très différentes. La \emph{probabilité a
posteriori} (section~\ref{sec:bic-vc}) est la cote dérivée du BIC, sous
une approximation par facteur de Bayes, qu'une ligne brisée s'ajuste
réellement mieux qu'une simple droite plutôt que par hasard. La
\emph{qualité d'ajustement} (section~\ref{sec:likelihood-intro}, déjà
dérivée) est la proximité de l'ajustement en ligne brisée confirmé aux
données, relative à un pire cas délibérément pessimiste~: $100\%$ à
l'ajustement parfait. Chacun de ces cinq nombres est un score $[0,1]$
borné et justifié, pas une statistique brute et dépendante d'une échelle
qu'il faudrait calibrer à l'œil~; quatre se lisent comme une preuve forte
proche de $100\%$, seul le p nul va dans l'autre sens, petit plutôt que
grand, exactement la convention habituelle d'une valeur $p$.
...
qu'un point par valeur de $k$ candidate, rarement plus d'une vingtaine.
...
contigus. Les 5 blocs par défaut laissent si peu de points par bloc
...
voisin doit respecter. Cette dépendance casse les algorithmes de
...
programmation dynamique. \code{research/multiknee/compare.py} mesure
directement la conséquence en aval, sur des courbes linéaires par
morceaux synthétiques ($n=100$, $25$ répliques par cellule vrai-$k$/bruit)~:
toute méthode fondée sur la programmation dynamique bat son équivalent
glouton, DP+mBIC\_sub atteignant $0{,}85$ d'exactitude sur $k$ contre
$0{,}60$ pour Greedy+mBIC\_sub, DP+BIC atteignant $0{,}65$ contre $0{,}49$
pour Greedy+BIC. Les erreurs d'engagement précoce de l'approche gloutonne
la poussent en particulier à \emph{surestimer} le nombre de points de
rupture à faible bruit, puisque corriger plus tard sa propre erreur de
placement ressemble, aux yeux d'un critère de sélection de modèle, à une
véritable structure supplémentaire méritant un point de rupture de plus.
C'est la motivation classique de la
Wild Binary Segmentation \citep{fryzlewicz2014}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/greedy_vs_dp_fr.png}
\caption{La programmation dynamique (bleu plein) contre l'approche
gloutonne (rouge pointillé) sur la même courbe, à $k=1$ et $k=2$ points
de rupture, propre (en haut) et avec un bruit $\sigma=0{,}15$ ajouté (en
bas). À $k=1$, les deux coïncident exactement~: une seule coupure n'a
rien à engager trop tôt. À $k=2$, sans bruit, la programmation dynamique
retrouve les vrais points de rupture avec une erreur quasi nulle, tandis
que la première coupure gloutonne, fixée à l'emplacement de $k=1$, l'empêche
d'atteindre le même ajustement à n'importe quelle étape ultérieure~:
l'échec concret que le texte ci-dessus décrit. Le bruit adoucit l'écart
sans le refermer.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/multiknee_comparison_fr.png}
\caption{À gauche~: exactitude sur $k$, classée, pour chaque combinaison
programmation dynamique (bleu) et gloutonne (rouge) avec les quatre
critères à plusieurs coudes que cette note dérive, $\mathrm{BIC}$,
$\mathrm{mBIC}$, $\mathrm{ICL}$ et le test séquentiel FWER, mesurée sur
le même banc d'essai synthétique. Toute méthode fondée sur la
programmation dynamique dépasse son équivalent glouton. À droite~: le
taux de faux positifs de ces mêmes dix méthodes sur des données
réellement plates, le $\mathrm{BIC}$ nu étant l'exception nette des
deux côtés, quelle que soit la stratégie de recherche.}
...
qu'\code{elbow-helper} fait tourner le localisateur sur toute une grille~:
...
\textbf{D'où vient la base du logarithme.} La sous-section~\ref{sec:likelihood-intro}
dérive, à partir de la vraisemblance gaussienne brute $L(\theta,\sigma^2)$,
que la log-vraisemblance négative

\begin{equation}
\mathrm{NLL} = n \ln\!\left(\frac{RSS}{n}\right) + n(1 + \ln 2\pi)
\end{equation}

est en logarithme naturel par construction, héritée de l'$\exp$ propre à
la densité gaussienne, pas choisie librement, et que ses cas extrêmes se
lisent mieux via $\mathrm{MSE}_{\text{pire}}$, une référence finie
délibérément pessimiste, plutôt que via $\mathrm{MSE} \to \infty$ ou la
moyenne d'échantillon, trop facile à battre. Toute formule de la famille
$\mathrm{BIC}$ dans cette note, y compris celle ci-dessous, s'ouvre avec
ce même terme $n\ln(RSS/n)$.

\textbf{Formule.} La correction de Schwarz au problème de surapprentissage
du paragraphe Intuition ajoute une pénalité de complexité, $p \ln n$,
par-dessus ce même $\mathrm{NLL}$, via une approximation de Laplace de la
vraisemblance marginale du modèle, elle aussi menée en logarithme naturel
de bout en bout. En laissant tomber la constante additive $n(1+\ln 2\pi)$,
qui s'annule quand on compare deux modèles sur les mêmes données~:
...
\citep{hastie2009esl} \citep{bishop2006prml}. Toute quantité en aval
hérite de cette même base naturelle par construction~: le $\mathrm{BIC}(k)$
à plusieurs coudes de la section~\ref{sec:several-bic}, le $\mathrm{mBIC}$
et l'ICL sont chacun construits en ajoutant d'autres termes de pénalité à
ce même $\mathrm{NLL}$, jamais en redérivant la vraisemblance depuis zéro.
Changer cette base n'importe où dans cette chaîne reviendrait à redériver
la densité gaussienne elle-même dans cette base, pas à simplement
redimensionner le nombre final.
...
La conséquence n'est pas que théorique~: la figure~\ref{fig:multiknee-comparison}
mesure un taux de faux positifs de $27\,\%$ pour ce critère exact
(DP+BIC) sur des données sans point de rupture réel, et seulement
$0{,}65$ de précision globale exacte en $k$, le pire des quatre critères
fondés sur la programmation dynamique testés
...
\section{L'ICL~: des modèles de mélange aux points de rupture, avec un \emph{bug} trouvé et corrigé au passage}
\label{sec:icl}
...
\textbf{Un \emph{bug} que les tests ont attrapé, non un choix de conception.}
...
correctement $k = 0$~; la figure~\ref{fig:multiknee-comparison} montre
qu'une fois corrigée, elle se situe dans la même tranche de performance
que le mBIC soustractif ($0{,}82$ contre $0{,}85$ de précision globale)
\citep{biernacki2000}.

\section{Un test séquentiel à taux d'erreur par famille contrôlé}
\label{sec:fwer}
...
taux d'erreur par famille réel (FWER, family-wise error rate), la
probabilité d'accepter au moins un point de rupture parasite n'importe où
dans la séquence, au plus égal au $\alpha$ nominal, jamais supérieur.

La figure~\ref{fig:multiknee-comparison} montre que cette approche
atteint un taux de faux positifs de $0\,\%$ à $\text{true } k = 0$ et
une précision globale de $0{,}82$, dans la même tranche que le mBIC
soustractif et l'ICL corrigé. Deux
...
la barrière séquentielle à taux d'erreur par famille contrôlé (section~20)
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/sigmoid_staircase_fr.png}
...
d'erreur par famille contrôlé divise $\alpha$ par $k_{\max}$, si bien que la
...
appliquée courte~: la règle du localisateur, « le dernier point avant la
...
\section{Un exemple travaillé~: une courbe de saturation exponentielle}

\code{examples/exponential\_saturation.py} fait tourner \code{robust\_knee}
sur une forme qu'aucun exemple ci-dessus n'a encore couverte~: $y = 1 -
e^{-t/\tau}$, une tension de charge de circuit RC, une courbe
d'apprentissage, une saturation dose-réponse, toute grandeur qui monte vite
puis s'aplatit vers un plafond. Contrairement à un vrai coude en ligne
brisée, cette courbe n'a de véritable rupture de pente nulle part~: elle
est lisse et indéfiniment dérivable, un test de résistance délibéré pour un
localisateur construit pour trouver des ruptures dans des tendances
linéaires par morceaux.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/exponential_saturation_fr.png}
\caption{La courbe de charge RC $y = 1 - e^{-t/\tau}$ avec un bruit léger,
et le coude détecté par \code{elbow\_helper.robust\_knee} (ligne rouge
pointillée et bulle) à $t/\tau \approx 1.98$.}
\end{figure}

Le piège pratique que révèle cet exemple est un décalage entre deux
lectures raisonnables de « où la courbe s'aplatit ». La constante de temps
$\tau$ des manuels marque $1 - e^{-1} \approx 63\%$ de la montée totale, un
repère propre et de forme close. Sur chaque tirage testé,
\code{robust\_knee} se place au contraire systématiquement autour de
$t/\tau \approx 1{,}9$--$2{,}0$, ce qui correspond à $1 - e^{-2} \approx
85\%$, la même convention « pratiquement stabilisée » utilisée
informellement en électronique RC et en automatique. Un œil humain lit la
courbe de la même façon~: à $\tau$, elle monte encore visiblement, pas
encore plate. Ce n'est pas un défaut à corriger~: c'est ce que signifie
« le dernier point avant la pliure » pour une courbe qui ne cesse jamais
vraiment de plier. Le coude localisé dépend aussi de la fenêtre
d'observation, ici $t \in [0, 5\tau]$, puisque la décision porte sur
l'écart maximal à la corde qui couvre la plage réellement fournie, pas sur
une propriété de $\tau$ seul.

\section{Un exemple travaillé~: latence de file d'attente et coude de dimensionnement}

\code{examples/queueing\_latency.py} fait tourner \code{robust\_knee} sur
une forme qu'aucun exemple ci-dessus ne prend~: le temps de réponse d'une
file d'attente à un seul serveur croît comme $1/(1-\rho)$ en fonction de
son utilisation $\rho$, un emballement classique de type M/M/1. La latence
reste proche du temps de service de base sur la majeure partie de la
plage, puis grimpe brutalement à mesure que $\rho$ approche 1. Cette
courbe est convexe et croissante plutôt que les formes concaves et
saturantes vues jusqu'ici~: le coude ne marque pas un plafond mais une
accélération, le point au-delà duquel un peu plus de charge coûte
beaucoup plus de latence.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/queueing_latency_fr.png}
\caption{Latence p99 en fonction de l'utilisation $\rho$, avec le coude
détecté par \code{elbow\_helper.robust\_knee} (ligne rouge pointillée et
bulle) à $\rho \approx 0{,}605$.}
\end{figure}

Le coude se situe à $\rho \approx 0{,}60$, bien avant l'emballement
mathématique à $\rho = 1$, pour une latence d'environ 2,5 fois le temps de
service de base. Une file d'attente paie déjà un coût réel à 60\%
d'utilisation~: la courbe ne paraît plate jusque-là que parce que
$1/(1-\rho)$ reste petit en échelle absolue. Dimensionner la capacité sur
« est-ce que ça a encore l'air à peu près plat » plutôt que sur ce coude
localisé est une façon courante et évitable de voir un plan de capacité
manquer de marge plus tôt que prévu.

\section{Un exemple travaillé~: dimensionnement de cache et rendements décroissants}

\code{examples/cache\_hit\_rate.py} répond avec le même outil à une
question familière d'infrastructure~: quelle taille donner à un cache~?
Pour un jeu de données dont la popularité suit à peu près une loi de
puissance, le taux de succès en fonction de la taille du cache $C$ suit
une forme de Michaelis-Menten, $C/(C+K)$, où $K$ est la taille de cache à
laquelle la moitié du jeu de données actif est déjà en mémoire~: des gains
rapides au début, puis une longue traîne qui s'aplatit. C'est concave et
croissant, la même famille large que la courbe de saturation
exponentielle deux sections plus haut, mais construite à partir d'une
fonction rationnelle plutôt que d'une exponentielle, et répondant à une
question de coût plutôt qu'à une question de constante de temps physique.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/cache_hit_rate_fr.png}
\caption{Taux de succès en fonction de la taille du cache, avec le coude
détecté par \code{elbow\_helper.robust\_knee} (ligne rouge pointillée et
bulle) à 691 éléments, soit $3{,}46\times K$.}
\end{figure}

Le coude se situe autour de $3{,}5K$, pas à $K$ lui-même. $K$ ne marque
que le point de 50\% de taux de succès, une propriété de l'algèbre de la
formule, pas de l'endroit où le rendement \emph{marginal} de la taille du
cache cesse réellement d'être rentable. Acheter du cache jusqu'à $K$
laisse beaucoup de taux de succès bon marché sur la table~; acheter
au-delà de $3{,}5K$ environ dépense de la mémoire réelle pour une part
de plus en plus mince de succès, exactement le chiffre dont une
discussion de dimensionnement de capacité a besoin et dispose rarement
sous forme close.

\section{Un exemple travaillé~: quand arrêter les tests}

\code{examples/bug\_discovery\_rate.py} tourne la même famille
convexe-décroissante que les exemples du k-means et de l'ACP vers une
question d'ingénierie de release plutôt que de clustering ou de
dimensionnalité. Les modèles de croissance de fiabilité décrivent la
découverte de défauts pendant les tests comme rapide au début (les bogues
faciles, à fort impact) puis lente (les bogues rares et profonds)~: une
courbe qui chute fortement, plie, puis s'aplatit en une longue traîne
basse.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/bug_discovery_rate_fr.png}
\caption{Bogues trouvés par jour de test, avec le coude détecté par
\code{elbow\_helper.robust\_knee} (ligne rouge pointillée et bulle) au
jour 10,7, un jour après la vraie rupture au jour 10.}
\end{figure}

Le coude localisé se situe environ un jour après la vraie rupture, le même
petit décalage systématique « un à deux points après la vraie pliure » que
montre l'exemple du diagramme des éboulis de l'ACP plus haut. Lire \code{knee\_x} comme « arrêter les tests
exactement ici » plutôt que comme « le taux de découverte s'est maintenant
vraiment aplati, à un jour ou deux près » accorde à l'estimation
ponctuelle plus de confiance qu'elle n'en revendique réellement.
