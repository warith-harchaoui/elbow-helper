...
\textbf{Du $\Delta\mathrm{BIC}$ à un score borné.} Un $\Delta\mathrm{BIC} =
\mathrm{BIC}_{\text{simple}} - \mathrm{BIC}_{\text{bris\'ee}}$ brut n'a pas
de plafond naturel~: « $10$ » et « $282$ » favorisent tous deux la ligne
bris\'ee, mais le nombre lui-m\^eme ne dit rien du degr\'e de confiance, et
son \'echelle d\'epend de la taille d'\'echantillon et du bruit. La figure
de diagnostic (\code{elbow\_helper.plotting}) affiche \`a la place une
probabilit\'e, un score qui doit tomber dans $[0, 1]$, avec $0$ et $1$
comme cas extr\^emes~: confiance totale dans la droite simple, ou dans la
ligne bris\'ee. C'est cette exigence qui fixe la base du logarithme, pas un
choix libre~: \citet{kassraftery1995} (\'eq.~4) montrent que
$\Delta\mathrm{BIC} \approx 2 \ln(\mathrm{BF})$, o\`u $\mathrm{BF}$ est le
facteur de Bayes entre les deux mod\`eles, et le $\ln$ \`a droite est
exactement le m\^eme logarithme naturel d\'ej\`a utilis\'e pour construire
le $\mathrm{BIC}$ ci-dessus, h\'erit\'e de sa d\'efinition en
$-2\ln(\hat L)$. Annuler ce $\ln$ demande donc l'inverse qui lui
correspond, $\exp$ en base $e$, pas un simple changement d'\'echelle par
$\ln 10$ ou $\ln 2$, qui ne ferait que renommer un nombre non born\'e sans
le borner~:
...
Sous des a priori \'egaux, les cotes a posteriori valent $\mathrm{BF}$,
donc la probabilit\'e a posteriori de la ligne bris\'ee est
$\mathrm{BF} / (1 + \mathrm{BF})$, exactement la sigmo\"ide logistique de
$\Delta\mathrm{BIC} / 2$~:

\begin{equation}
P(\text{ligne bris\'ee} \mid \text{donn\'ees}) = \frac{1}{1 + \exp(-\Delta\mathrm{BIC}/2)}
\end{equation}

born\'ee dans $[0, 1]$ par construction, \`a $0.5$ quand
$\Delta\mathrm{BIC} = 0$ (aucune preuve dans un sens ou l'autre), et
saturant en douceur vers les extr\^emes $0$/$1$ \`a mesure que la preuve
s'accumule, dans un sens ou l'autre, plut\^ot que de croître sans limite
comme le $\Delta\mathrm{BIC}$ brut.
