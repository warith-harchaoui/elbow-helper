...
Partons de la vraisemblance brute, avant tout logarithme, et prenons-la
\emph{par observation} dès le départ~: la moyenne géométrique des $n$
densités gaussiennes individuelles, plutôt que leur produit brut. Sous un
modèle génératif $y_i = f(x_i;\theta) + \varepsilon_i$ avec $\varepsilon_i$
...
\textbf{Heuristiques de coude et d'elbow.} Le Kneedle de Satopaa,
Albrecht, Irwin et Raghavan \citep{satopaa2011} repère un coude via la
même géométrie de distance à la corde que la section~\ref{sec:diff-curve}
construit, comme une estimation ponctuelle sans machinerie de confiance.
Le repéreur d'\code{elbow-helper} emprunte cette intuition géométrique et
l'enveloppe dans la procédure de décision que cette note dérive.

\textbf{Détection de points de rupture et segmentation.} L'algorithme de
programmation dynamique élaguée de Killick, Fearnhead et Eckley, Pruned
Exact Linear Time (PELT) \citep{killick2012}, atteint une segmentation
exacte et optimale à coût linéaire, le même principe de sous-structure
optimale sur lequel repose aussi la section~\ref{sec:dp}, restreint ici à
un petit $k_{\max}$ plutôt qu'au $k$ non borné piloté par pénalité de
PELT. La segmentation binaire
sauvage de Fryzlewicz \citep{fryzlewicz2014} est l'alternative gloutonne
à la programmation dynamique exacte derrière la comparaison de la
section~\ref{sec:greedy}. Yao \citep{yao1988} a le premier analysé
l'estimation du nombre de points de rupture via le critère de Schwarz,
l'ancêtre direct du critère $\mathrm{BIC}(k)$ à plusieurs coudes de la
section~\ref{sec:several-bic}.

\textbf{Critères de sélection de modèle.} Schwarz \citep{schwarz1978} est
à l'origine du $\mathrm{BIC}$. Zhang et Siegmund \citep{zhangsiegmund2007}
corrigent la sous-facturation du $\mathrm{BIC}$ pour la liberté de
placement des points de rupture, le $\mathrm{mBIC}$ de la
section~\ref{sec:mbic}. Biernacki, Celeux et Govaert
\citep{biernacki2000} introduisent la vraisemblance complétée intégrée
pour les modèles de mélange, adaptée aux points de rupture dans la
section~\ref{sec:icl} via la distribution a posteriori exacte de Rigaill,
Lebarbier et Robin par passage avant-arrière sur les segmentations
\citep{rigaill2012}, encore accélérée par Cleynen, Luong, Rigaill et Nuel
\citep{cleynen2013}. Kass et Raftery \citep{kassraftery1995} fournissent
la voie du facteur de Bayes, d'une différence de vraisemblance brute
jusqu'à une probabilité a posteriori, la normalisation alternative notée
ci-dessus.

\textbf{Tests séquentiels et multi-échelles.} Davies \citep{davies1977}
traite le test d'hypothèse quand un paramètre de nuisance, ici
l'emplacement du point de rupture, n'existe que sous l'hypothèse
alternative, un problème auquel le test séquentiel de la
section~\ref{sec:fwer} se heurte directement. Le Simultaneous MUlti-scale
Change-point Estimator de Frick, Munk et Sieling (SMUCE)
\citep{frick2014} donne cette même garantie à chaque échelle à la fois,
cité aux côtés de celui-ci.

\textbf{Statistique robuste et classique.} La corrélation de rang de
Spearman \citep{spearman1904} filtre la présence d'une tendance monotone
avant toute recherche de coude. Theil \citep{theil1950} et Sen
\citep{sen1968} fournissent l'estimateur de pente robuste aux valeurs
aberrantes derrière le contraste de Theil-Sen. Le bootstrap d'Efron
\citep{efron1979} est la méthode de rééchantillonnage derrière le test de
stabilité.

\textbf{Méthodologie.} L\'opez de Prado \citep{lopezdeprado2018financial}
défend longuement la validation croisée contiguë, non mélangée, sur des
données ordonnées et séquentielles, le raisonnement derrière la
validation croisée par blocs de ce projet.
