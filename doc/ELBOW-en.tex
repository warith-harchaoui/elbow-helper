...
root of $L_{\text{raw}}$. That departure from the textbook definition,
and why it is principled rather than a convenient rescaling, is the
subject of a companion note, \code{LIKELIHOOD-en.tex}, factored out
because that foundation is general and does not depend on curve-fitting
at all: Jensen's inequality and a temperature-indexed refinement borrowed
from equilibrium statistical mechanics, Cram\'er's large deviations and
Wald's martingale, a precise account of $L$ as a statistical estimator
with its bias and variance derived and verified by simulation, and how
the same construction reads on a categorical classification model
(with its own worked simulation) as well as the Gaussian regression model
derived below. This note builds directly on top of it: every
...
\citep{satopaa2011}, built for exactly the setting this note opens
...
Pruned Exact Linear Time (PELT) \citep{killick2012}, sits on the exact
side, a penalized dynamic program pruned so that provably suboptimal
candidate boundaries are discarded before they are even scored,
recovering the same optimal segmentation an unpruned search would find
at linear rather than quadratic cost, with $k$ itself set by the penalty
rather than fixed in advance. Section~\ref{sec:dp} rests on the same
optimal-substructure argument, restricted here to a small, explicitly
bounded $k_{\max}$ (few real curves plausibly hold more than a handful
of genuine bends) rather than PELT's unbounded, penalty-driven $k$,
which is exactly what keeps the search small enough to run underneath
this project's bootstrap and permutation layers, something PELT itself
has no equivalent of. Fryzlewicz's Wild Binary Segmentation
\citep{fryzlewicz2014} sits on the other side of that line: a recursive,
greedy splitting procedure over randomly drawn subintervals, faster than
exact search but liable to freeze in an early bad cut it can never
reconsider, exactly the failure mode Section~\ref{sec:greedy}
demonstrates on a constructed example and the reason this project scores
greedy segmentation as the faster, worse alternative rather than the
default choice. Yao \citep{yao1988} was first to tie changepoint counting
...
to a model's marginal likelihood that charges a fixed $\frac{1}{2}\ln n$
per free parameter, correct for regular parametric families but silently
assuming absent exactly the irregularity a breakpoint's location
introduces, an assumption Section~\ref{sec:several-bic} shows failing
concretely rather than just in principle. Zhang and Siegmund
...
sign ambiguity this note resolves empirically rather than by assumption
(again Section~\ref{sec:mbic}). Biernacki, Celeux and Govaert
...
discrete segmentation space, structurally the same recursion as a
hidden Markov model's forward-backward algorithm run over segmentations
rather than hidden states, later accelerated by Cleynen, Luong, Rigaill
and Nuel \citep{cleynen2013}. Section~\ref{sec:icl}'s account of that
...
breakpoint's location is a nuisance parameter that exists only under the
alternative, so the usual likelihood-ratio asymptotics do not apply,
exactly the pivot problem Section~\ref{sec:fwer}'s sequential test sidesteps
by using a permutation null instead of an asymptotic one. Frick, Munk
...
directly by simulation rather than by asymptotic theory. Both were
...
exactly the setting where asymptotic normality is hardest to justify
and resampling easiest.

\textbf{Methodology.} L\'opez de Prado \citep{lopezdeprado2018financial}
argues at length, from financial time series rather than curve fitting,
for contiguous rather than shuffled cross-validation blocks whenever
...
convex-decreasing case. It is worth seeing end to end: run k-means for
...
intensity, among others) and a real cultivar out of three, never used
for fitting, only for comparison after the fact. The thirteen
measurements are standardized, then exactly the same k-means code as
...
That is not a bug; it is Section~\ref{sec:persistence} doing exactly
...
once and looked up here rather than recomputed, adds the cost of one
final segment, and the cheapest of the three candidates wins. A real
...
the textbook motivation for Wild Binary Segmentation \citep{fryzlewicz2014}.
...
the FWER sequential test, measured on the same synthetic benchmark.
...
and the \emph{Laplace approximation} is the general recipe for
...
Bayes factor between the two models; the $\ln$ on the right is the
...
balanced case, exactly the concavity of $\ln$ making a small segment
...
lower-is-better criterion, rewarding exactly the split that should cost
...
This example shares Section~\ref{sec:kmeans-example}'s practical trap exactly: a scree plot is
...
This is not a bug to fix: it is what ``last point before the bend'' means
for a curve that never truly stops bending: the located knee also depends
...
roughly $3.5K$ spends real memory for a shrinking sliver of hits, exactly
