...
from generate_experiment_figures import (  # noqa: E402
    ...
    svg_open, xml_escape, _write_and_rasterize, pow10_label,
)
...
def build_temperature_sweep_svg() -> str:
    ...
    parts.append("<title id=\"ts-title\">Jensen's inequality and the temperature sweep</title>")
    parts.append("<desc id=\"ts-desc\">Bar chart of p with its geometric and arithmetic means, "
    ...
    parts.append(f'<text x="60" y="82" font-size="15" fill="{SUBTLE}">'
    ...
    parts.append(f'<text x="{bx:.1f}" y="{py-18:.1f}" font-size="16" font-weight="700" fill="{INK}">'
                 f'bars vs. their two means</text>')
    for v in (0.0, 0.2, 0.4, 0.6):
        gy = sy_l(v)
        parts.append(f'<line x1="{bx:.1f}" y1="{gy:.1f}" x2="{bx+panel_w:.1f}" y2="{gy:.1f}" '
                     f'stroke="{GRID}" stroke-width="1.2"/>')
        parts.append(f'<text x="{bx-12:.1f}" y="{gy+5:.1f}" font-size="13" '
                     f'font-family="Roboto Mono, monospace" fill="{INK}" text-anchor="end">{v:.1f}</text>')
    ax_bottom_l = py + ph
    parts.append(f'<line x1="{bx:.1f}" y1="{ax_bottom_l:.1f}" x2="{bx+panel_w:.1f}" y2="{ax_bottom_l:.1f}" '
                 f'stroke="{INK}" stroke-width="1.5"/>')
    parts.append(f'<line x1="{bx:.1f}" y1="{py:.1f}" x2="{bx:.1f}" y2="{ax_bottom_l:.1f}" '
                 f'stroke="{INK}" stroke-width="1.5"/>')
    ...
    for i, pk in enumerate(CAT_P):
        cx = bx + slot * (i + 0.5)
        top = sy_l(float(pk))
        parts.append(f'<rect x="{cx-bar_w/2:.1f}" y="{top:.1f}" width="{bar_w:.1f}" '
                     f'height="{ax_bottom_l-top:.1f}" fill="{BLUE}" fill-opacity="0.75" '
                     f'stroke="{BLUE}" stroke-width="1.3"/>')
        parts.append(f'<text x="{cx:.1f}" y="{ax_bottom_l+22:.1f}" font-size="13" fill="{INK}" '
                     f'text-anchor="middle">k={i+1}</text>')
    parts.append(f'<text x="{bx+panel_w/2:.1f}" y="{ax_bottom_l+48:.1f}" font-size="14" fill="{INK}" '
                 f'text-anchor="middle">category</text>')

    gm_y, am_y = sy_l(geo_mean), sy_l(arith_mean)
    parts.append(f'<line x1="{bx:.1f}" y1="{gm_y:.1f}" x2="{bx+panel_w:.1f}" y2="{gm_y:.1f}" '
                 f'stroke="{RED}" stroke-width="2" stroke-dasharray="7 4"/>')
    parts.append(f'<text x="{bx+panel_w-4:.1f}" y="{gm_y-7:.1f}" font-size="12.5" fill="{RED}" '
                 f'text-anchor="end">geometric mean {geo_mean:.3f}</text>')
    parts.append(f'<line x1="{bx:.1f}" y1="{am_y:.1f}" x2="{bx+panel_w:.1f}" y2="{am_y:.1f}" '
                 f'stroke="{ORANGE}" stroke-width="2" stroke-dasharray="7 4"/>')
    parts.append(f'<text x="{bx+panel_w-4:.1f}" y="{am_y-7:.1f}" font-size="12.5" fill="{ORANGE}" '
    ...
    def sx_r(b: float) -> float:
        return px_right + (math.log10(b) - log_b_min) / (log_b_max - log_b_min) * panel_w
    ...
    parts.append(f'<text x="{px_right+panel_w/2:.1f}" y="{ax_bottom_r+50:.1f}" font-size="14" fill="{INK}" '
    ...
    parts.append(f'<circle cx="{b1x:.1f}" cy="{b1y:.1f}" r="6" fill="{RED}" stroke="#FFFFFF" stroke-width="1.4"/>')
    parts.append(f'<text x="{b1x+10:.1f}" y="{b1y-10:.1f}" font-size="12.5" fill="{RED}">&#946;=1</text>')
    parts.append(f'<text x="{px_right+8:.1f}" y="{sy_r(floor)-8:.1f}" font-size="12" fill="{SUBTLE}">'
                 f'floor 1/K={floor:.2f}</text>')
    parts.append(f'<text x="{px_right+8:.1f}" y="{sy_r(ceil_)+18:.1f}" font-size="12" fill="{SUBTLE}">'
...
def simulate_classification(rng: np.random.Generator) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    centroids = np.array([[0.0, 0.0], [3.2, 0.3], [1.5, 2.6]])
    covs = [np.array([[1.1, 0.3], [0.3, 1.0]]) * 0.9,
...
def build_classification_density_svg(rng: np.random.Generator) -> str:
    ...
    parts.append("<title id=\"cd-title\">Classification confidence, geometric mean of the true-class probabilities</title>")
    parts.append("<desc id=\"cd-desc\">45 simulated points from 3 overlapping classes, dot size the "
    ...
    parts.append(f'<rect x="{px_left:.1f}" y="{py:.1f}" width="{panel_w}" height="{ph}" '
                 f'fill="none" stroke="{GRID}" stroke-width="1.2"/>')

    for c, col in zip(centroids, colors):
        parts.append(f'<circle cx="{sx(c[0]):.1f}" cy="{sy(c[1]):.1f}" r="{1.35*scale:.1f}" '
    ...
    for (x, y), lab, prob in zip(pts, labels, true_probs):
        ...
        col = colors[int(lab)]
        parts.append(f'<circle cx="{sx(x):.1f}" cy="{sy(y):.1f}" r="{r:.1f}" fill="{col}" '
    ...
    for i, (col, name) in enumerate(zip(colors, ["class 1", "class 2", "class 3"])):
        ...
        parts.append(f'<text x="{lx+14:.1f}" y="{ly+4:.1f}" font-size="13" fill="{INK}">{name}</text>')
    ...
    for v in (0.0, 0.25, 0.5, 0.75, 1.0):
        ...
        parts.append(f'<text x="{px_right-12:.1f}" y="{gy+5:.1f}" font-size="12.5" '
                     f'font-family="Roboto Mono, monospace" fill="{INK}" text-anchor="end">{v:.2f}</text>')
    ax_bottom = py + ph
    parts.append(f'<line x1="{px_right:.1f}" y1="{ax_bottom:.1f}" x2="{px_right+panel_w:.1f}" y2="{ax_bottom:.1f}" '
                 f'stroke="{INK}" stroke-width="1.5"/>')
    parts.append(f'<line x1="{px_right:.1f}" y1="{py:.1f}" x2="{px_right:.1f}" y2="{ax_bottom:.1f}" '
                 f'stroke="{INK}" stroke-width="1.5"/>')
    parts.append(f'<text x="{px_right+panel_w/2:.1f}" y="{ax_bottom+30:.1f}" font-size="13.5" fill="{INK}" '
    ...
    for i, v in enumerate(sorted_probs):
        cx = sx2(float(i))
        top = sy2(float(v))
        parts.append(f'<rect x="{cx-bw/2:.1f}" y="{top:.1f}" width="{bw:.1f}" height="{ax_bottom-top:.1f}" '
                     f'fill="{BLUE}" fill-opacity="0.75"/>')

    gm_y = sy2(geo_mean)
    parts.append(f'<line x1="{px_right:.1f}" y1="{gm_y:.1f}" x2="{px_right+panel_w:.1f}" y2="{gm_y:.1f}" '
                 f'stroke="{RED}" stroke-width="2.2" stroke-dasharray="7 4"/>')
    parts.append(f'<text x="{px_right+8:.1f}" y="{gm_y-9:.1f}" font-size="13" fill="{RED}">'
...
def gaussian_pdf(y: np.ndarray, mean: float, sigma: float) -> np.ndarray:
    return (1.0 / (math.sqrt(2 * math.pi) * sigma)) * np.exp(-0.5 * ((y - mean) / sigma) ** 2)
...
def simulate_regression(rng: np.random.Generator, n: int = 24) -> dict:
    ...
    return dict(x=x, y=y, a=float(a), b=float(b), sigma_hat=sigma_hat, mse=mse, geo_mean=geo_mean)
...
def build_regression_density_svg(rng: np.random.Generator) -> str:
    ...
    parts.append("<title id=\"rd-title\">The likelihood as a geometric mean of curve heights</title>")
    parts.append("<desc id=\"rd-desc\">Synthetic regression data with the fitted line and four "
    ...
    parts.append(f'<text x="{m_left}" y="82" font-size="15" fill="{SUBTLE}">'
                 f'y = 2 + 0.5x + &#949;, &#949; ~ N(0, 0.9&#178;), fitted by least squares</text>')

    for v in np.linspace(math.ceil(y_min), math.floor(y_max), int(math.floor(y_max) - math.ceil(y_min)) + 1):
        gy = sy(float(v))
    ...
    for v in range(math.ceil(float(x.min())) - 1, math.floor(float(x.max())) + 2):
        ...
        parts.append(f'<line x1="{gx:.1f}" y1="{ax_bottom:.1f}" x2="{gx:.1f}" y2="{ax_bottom+5:.1f}" '
                     f'stroke="{INK}" stroke-width="1.2"/>')
        parts.append(f'<text x="{gx:.1f}" y="{ax_bottom+24:.1f}" font-size="13" '
                     f'font-family="Roboto Mono, monospace" fill="{INK}" text-anchor="middle">{v}</text>')
    parts.append(f'<text x="{px+plot_w/2-40:.1f}" y="{ax_bottom+48:.1f}" font-size="14" fill="{INK}" '
                 f'text-anchor="middle">x</text>')

    # fitted line, across the real data range only
    lx0, lx1 = float(x.min()) - 0.3, float(x.max()) + 0.3
    parts.append(f'<line x1="{sx(lx0):.1f}" y1="{sy(a+b*lx0):.1f}" x2="{sx(lx1):.1f}" y2="{sy(a+b*lx1):.1f}" '
                 f'stroke="{INK}" stroke-width="2"/>')

    # every point, small
    for xi, yi in zip(x, y):
        parts.append(f'<circle cx="{sx(float(xi)):.1f}" cy="{sy(float(yi)):.1f}" r="3.4" '
    ...
    for idx in picks:
        ...
        curve_pts = [(sx(x0) + d_ * bell_scale, sy(float(yy))) for yy, d_ in zip(y_grid, dens)]
        base_top = (sx(x0), sy(float(y_grid[0])))
        base_bot = (sx(x0), sy(float(y_grid[-1])))
        path_d = (f"M {base_top[0]:.1f} {base_top[1]:.1f} "
                  + " L ".join(f"{cx:.1f} {cy:.1f}" for cx, cy in curve_pts)
                  + f" L {base_bot[0]:.1f} {base_bot[1]:.1f} Z")
        parts.append(f'<path d="{path_d}" fill="{BLUE}" fill-opacity="0.16" stroke="{BLUE}" stroke-width="1.6"/>')
        parts.append(f'<line x1="{sx(x0):.1f}" y1="{sy(float(y_grid[0])):.1f}" x2="{sx(x0):.1f}" '
        ...
        parts.append(f'<line x1="{sx(x0):.1f}" y1="{sy(y0):.1f}" x2="{dot_x:.1f}" y2="{sy(y0):.1f}" '
                     f'stroke="{RED}" stroke-width="1.3" stroke-dasharray="3 3"/>')
        parts.append(f'<circle cx="{dot_x:.1f}" cy="{sy(y0):.1f}" r="5.5" fill="{RED}" '
                     f'stroke="#FFFFFF" stroke-width="1.2"/>')

    parts.append(f'<text x="{px+16:.1f}" y="{py+16:.1f}" font-size="13" fill="{SUBTLE}">'
...
def main() -> None:
    print("temperature sweep: geo_mean={:.4f} arith_mean={:.4f} floor={:.3f} ceiling={:.3f}".format(
        float(np.exp((CAT_P * np.log(CAT_P)).sum())), float((CAT_P**2).sum()),
        1.0 / len(CAT_P), float(CAT_P.max())))
    ...
    d = simulate_regression(rng)
    print(f"regression: a={d['a']:.3f} b={d['b']:.3f} mse={d['mse']:.3f} L={d['geo_mean']:.3f}")
    rng = np.random.default_rng(3)
