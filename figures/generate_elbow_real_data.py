...
_SHORT_CURVE_CONFIG = RobustKneeConfig(
    min_samples=6, min_side_points=2, min_consecutive_scales=2,
    min_sensitivity_support=0.5, min_slope_contrast=0.15,
    min_bic_improvement=3.0, min_cv_improvement=0.02,
    bootstrap_replicates=200, min_bootstrap_detection_rate=0.7,
    max_ci90_width=0.2, min_primary_cluster_rate=0.6,
    max_bootstrap_median_shift=0.08, null_replicates=200,
    max_null_p_value=0.05, random_seed=0, cv_folds=3,
)
...
def _kmeans_inertia(x: np.ndarray, k: int, seed: int, n_init: int = 15, max_iter: int = 100) -> float:
...
def compute() -> Dict[str, object]:
    ...
    return {
        "n": X.shape[0], "n_features": X.shape[1],
        "ks": ks, "inertias": inertias, "km_result": type(r_km).__name__,
        "km_reason": getattr(r_km, "reason", None),
        "comps": comps, "eigvals": eigvals, "pca_result": type(r_pca).__name__,
        "pca_reason": getattr(r_pca, "reason", None),
    }


def _single_panel_svg(lang: str, title: str, sub: str, xlabel: str, ylabel: str,
                       xs: np.ndarray, ys: np.ndarray, note: str,
                       candidate_x: float = None, candidate_label: str = "") -> str:
    ...
    parts.append(f'<text x="{m_left}" y="52" font-size="23" font-weight="700" fill="{INK}">'
    ...
    for v in y_ticks:
        ...
        parts.append(f'<text x="{plot_x-14:.1f}" y="{gy+5:.1f}" font-size="14" '
    ...
    for v in xs:
        ...
        parts.append(f'<text x="{gx:.1f}" y="{ax_bottom+26:.1f}" font-size="13" '
                     f'font-family="Roboto Mono, monospace" fill="{INK}" text-anchor="middle">{int(v)}</text>')
    parts.append(f'<text x="{plot_x+plot_w/2:.1f}" y="{ax_bottom+58:.1f}" font-size="16" fill="{INK}" '
                 f'text-anchor="middle">{xml_escape(xlabel)}</text>')
    ytx, yty = 40, plot_y + plot_h / 2
    parts.append(f'<text x="{ytx}" y="{yty:.1f}" font-size="15" fill="{INK}" text-anchor="middle" '
                 f'transform="rotate(-90 {ytx} {yty:.1f})">{xml_escape(ylabel)}</text>')

    if candidate_x is not None:
        gx = sx(candidate_x)
        parts.append(f'<line x1="{gx:.1f}" y1="{plot_y:.1f}" x2="{gx:.1f}" y2="{ax_bottom:.1f}" '
                     f'stroke="{SUBTLE}" stroke-width="1.6" stroke-dasharray="3 5"/>')
        parts.append(f'<text x="{gx+8:.1f}" y="{plot_y+22:.1f}" font-size="13" fill="{SUBTLE}">'
    ...
    parts.append(f'<text x="{plot_x+plot_w-10:.1f}" y="{plot_y+22:.1f}" font-size="13.5" '
...
def main() -> None:
    print("running real k-means + PCA on the Wine dataset via elbow_helper.robust_elbow ...")
    res = compute()
    ...
    for lang in ("fr", "en"):
        ...
        svg_km = _single_panel_svg(
            lang, t["kmeans_title"], t["kmeans_sub"], t["kmeans_x"], t["kmeans_y"],
            res["ks"], res["inertias"], t["kmeans_note"],
            candidate_x=3.0, candidate_label=t["kmeans_candidate"],
        )
        _write_and_rasterize(svg_km, f"kmeans_wine_{lang}", 1000)

        svg_pca = _single_panel_svg(
            lang, t["pca_title"], t["pca_sub"], t["pca_x"], t["pca_y"],
            res["comps"], res["eigvals"], t["pca_note"],
        )
