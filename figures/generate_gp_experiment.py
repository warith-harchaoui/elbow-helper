...
def log_marginal_likelihood(
    xs: np.ndarray, y: np.ndarray, sigma_f2: float, ell: float, sigma_n2: float
) -> float:
    n = len(y)
    ...
    return float(
        -0.5 * y @ alpha - np.sum(np.log(np.diag(L))) - 0.5 * n * math.log(2 * math.pi)
    )
...
def predict(
    x: np.ndarray, y: np.ndarray, fit: Dict[str, object], x_grid: np.ndarray
...
def build_posterior_svg(x: np.ndarray, y: np.ndarray, fit: Dict[str, object]) -> str:
    ...
    sub = (
        f"50 real blocks, median income vs. median house value; "
    ...
    parts.append(
        f'<text x="{px + plot_w / 2:.1f}" y="{ax_bottom + 50:.1f}" font-size="14.5" fill="{INK}" '
    ...
    band_d = (
        "M "
        + " L ".join(f"{cx:.1f} {cy:.1f}" for cx, cy in top_pts)
        + " L "
        + " L ".join(f"{cx:.1f} {cy:.1f}" for cx, cy in reversed(bot_pts))
        + " Z"
    )
    ...
    for xv, yv in zip(x, y):
        parts.append(
            f'<circle cx="{sx(float(xv)):.1f}" cy="{sy(float(yv)):.1f}" r="4.5" '
            f'fill="{INK}" fill-opacity="0.75"/>'
        )

    lx, ly = px + 16, py + 16
    parts.append(
        f'<rect x="{lx - 8:.1f}" y="{ly - 14:.1f}" width="230" height="52" '
        f'fill="#FFFFFF" fill-opacity="0.92" stroke="{GRID}" stroke-width="1"/>'
    )
    parts.append(
        f'<circle cx="{lx + 4:.1f}" cy="{ly:.1f}" r="4.5" fill="{INK}" fill-opacity="0.75"/>'
    )
    parts.append(
        f'<text x="{lx + 16:.1f}" y="{ly + 4:.1f}" font-size="12.5" fill="{INK}">50 real blocks</text>'
    )
    parts.append(
        f'<line x1="{lx - 2:.1f}" y1="{ly + 22:.1f}" x2="{lx + 22:.1f}" y2="{ly + 22:.1f}" '
        f'stroke="{BLUE}" stroke-width="2.6"/>'
    )
    parts.append(
        f'<text x="{lx + 30:.1f}" y="{ly + 26:.1f}" font-size="12.5" fill="{INK}">posterior mean</text>'
    )
    parts.append(
        f'<rect x="{lx - 2:.1f}" y="{ly + 32:.1f}" width="24" height="10" '
        f'fill="{BLUE}" fill-opacity="0.16"/>'
    )
    parts.append(
        f'<text x="{lx + 30:.1f}" y="{ly + 41:.1f}" font-size="12.5" fill="{INK}">±2σ band</text>'
    )
...
def build_lml_svg(fit: Dict[str, object]) -> str:
    ...
    parts.append(
        f'<text x="{sx(1.3):.1f}" y="{sy(y_min + 0.2 * (y_max - y_min)):.1f}" font-size="12.5" '
...
def main() -> None:
    ...
    _write_and_rasterize(
        build_posterior_svg(x, y, fit), "gp_posterior_housing_en", 1050
    )
