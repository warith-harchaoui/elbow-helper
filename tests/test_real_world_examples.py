...
figures and prose in ELBOW-en.tex / ELBOW-fr.tex: the k-means inertia elbow and
...
# needed instead; see ELBOW-en.tex / ELBOW-fr.tex for this trade-off.
...
def kmeans_inertia_curve(seed, true_k=8, k_max=24, n_per_cluster=50):
    ...
    ELBOW-en.tex / ELBOW-fr.tex's k-means figure.
    """
    rng = np.random.default_rng(seed)
    grid = np.array(
...
def pca_scree_curve(seed, n_samples=300, true_d=6, n_noise_dims=19, signal_scale=8.0,
    ...
    eigenvalues never actually take. Shared with ELBOW-en.tex / ELBOW-fr.tex's
    PCA figure.
    """
    rng = np.random.default_rng(seed)
...
def test_pca_scree_elbow_detects_true_signal_dimension():
    ...
    for seed in range(5):
        ...
        # not exactly on it; see ELBOW-en.tex / ELBOW-fr.tex for this reading.
