...
FWER-gated multi-knee pipeline. See MATH-en.md sections 5-20 and
...
def sigmoid_staircase_curve(seed, n=200, n_steps=3, steepness=40.0, noise=0.02):
    ...
    ``MATH-en.md`` / ``MATH-fr.md``'s multi-knee figure, which runs
...
def alternating_slope_curve(seed, n=150, noise=0.02):
    ...
    down) needs no special handling. Shared with ``MATH-en.md`` /
    ``MATH-fr.md``'s alternating-slope figure.
    """
    rng = np.random.default_rng(seed)
    x = np.linspace(0, 1, n)
    y = np.piecewise(
...
def subtle_knee_curve(seed, n=150, noise=0.05):
    ...
    detection-boundary figure in ``MATH-en.md`` / ``MATH-fr.md``.
    """
    rng = np.random.default_rng(seed)
    x = np.linspace(0, 1, n)
...
def test_subtle_knee_detected_at_low_noise_mostly_abstains_at_high_noise():
    ...
    # "wrong in the honest direction" (see MATH-en.md section 16 / RESULTS.md).
...
def test_sigmoid_staircase_brackets_each_rise_with_a_breakpoint_pair():
    ...
    # effect; fwer_permutations must scale with k_max. See MATH-en.md
